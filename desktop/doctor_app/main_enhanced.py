...
class DoctorWindow(QMainWindow):
    def __init__(self,mode:ModeConfig):
        super().__init__(); self.mode=mode; self.db=LocalDatabase(); self.patient_mgr=PatientManager(self.db); self.current_case=None; self.live_patient=None; self.page_keys=[]; self.metric_labels={}; self.packet_count=0
    ...
    def _dashboard(self):
        ...
        for i,(a,b,c) in enumerate([("Patients in demo queue","8","Synthetic review cases"),("High signal tier","1","Synthetic tier, not clinical severity"),("CHRONO-PCOS cases","3","First disease-specific module"),("Live signal quality","—","Updates from processed stream")]): g.addWidget(card(a,b,c),0,i)
