...
Both workstations support DEMO MODE and LIVE SENSOR MODE. Desktop LIVE mode uses dynamically discovered USB serial ports and CRC-checked `$CP3` packets (the `$CP2`/`$CP` layouts still parse, see `docs/WIRE_FORMAT_CP3.md`).

## Live paths
```
ESP8266 sensor pod ── Wi-Fi TCP ── CP3 ──> Android/Desktop
ESP32-S3 sensor pod ── Wi-Fi TCP ── CP3 ──> Android/Desktop
Mega lab      ───── USB Serial ────────────── CP3 ──> Python/Desktop
```
...
Patient and Doctor remain native Kotlin + Compose + Material 3. Live mobile acquisition uses the validated CP3 TCP stream from the sensor pod; DEMO_DATA remains explicitly separated from live data.
