/*
  ...
  Canonical packet (CP3 — CP2 with the retired gsr field removed):
...
enum StatusBit {
  ...
  /* bit 4 was ST_GSR_SAT and is retired with the GSR channel */ ST_I2C_ERR=5,
  ST_LOW_QUALITY=6, ST_ECG_LEADS=7,
...
int micRaw=0,ecgRaw=-1,fsrRaw=-1;
...
void readAnalog(){ecgRaw=analogRead(ECG_PIN);fsrRaw=analogRead(FSR_PIN);buttonMask=0;if(digitalRead(BTN_MODE)==LOW)buttonMask|=1;if(digitalRead(BTN_BASE)==LOW)buttonMask|=2;if(digitalRead(BTN_POST)==LOW)buttonMask|=4;}
...
uint16_t status(){uint16_t s=statusBase;if(ppgOK){if(irValue<5000)s|=(1<<ST_PPG_ABSENT);if(irValue>250000UL||redValue>250000UL)s|=(1<<ST_PPG_SAT);}if(tempOK&&!(temp0>-20&&temp0<80))s|=(1<<ST_TEMP_ERR);if(digitalRead(ECG_LO_PLUS)||digitalRead(ECG_LO_MINUS))s|=(1<<ST_ECG_LEADS);if(micRms<2)s|=(1<<ST_MIC_LOW);if(fsrRaw<20||fsrRaw>1000)s|=(1<<ST_FSR_ARTIFACT);return s;}
void sendPacket(){char ax[12],ay[12],az[12],gx[12],gy[12],gz[12],t0[12],t1[12],lx[12],rt[12],hu[12],pr[12],mr[12],mp[12];dtostrf(ax_g,1,4,ax);dtostrf(ay_g,1,4,ay);dtostrf(az_g,1,4,az);dtostrf(gx_dps,1,3,gx);dtostrf(gy_dps,1,3,gy);dtostrf(gz_dps,1,3,gz);dtostrf(temp0,1,2,t0);dtostrf(temp1,1,2,t1);dtostrf(luxValue,1,1,lx);dtostrf(roomT,1,2,rt);dtostrf(humidity,1,1,hu);dtostrf(pressure,1,1,pr);dtostrf(micRms,1,2,mr);dtostrf(micPitchHz,1,1,mp);char p[280];uint16_t s=status();snprintf(p,sizeof(p),"$CP3,%lu,%lu,%lu,%s,%s,%s,%s,%s,%s,%s,%s,%d,%s,%s,%d,%d,%s,%s,%s,%s,%u,%u",millis(),(unsigned long)irValue,(unsigned long)redValue,ax,ay,az,gx,gy,gz,t0,t1,micRaw,mr,mp,ecgRaw,fsrRaw,lx,rt,hu,pr,buttonMask,s);uint8_t crc=xorCRC(p);Serial.print(p);Serial.print(',');if(crc<16)Serial.print('0');Serial.println(crc,HEX);}
...
#if 1
if(oledOK){display.clearDisplay();display.setTextSize(1);display.setTextColor(SSD1306_WHITE);display.setCursor(0,0);display.println("ENDO-TWIN MEGA LAB");display.print("IR:");display.println(irValue);display.print("T:");display.println(temp0);display.println("Research prototype");display.display();}
#endif
