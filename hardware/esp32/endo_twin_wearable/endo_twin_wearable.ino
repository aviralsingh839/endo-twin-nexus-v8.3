...
static constexpr uint8_t SDA_PIN=21, SCL_PIN=22, ONE_WIRE_BUS=18, GSR_PIN=34, STATUS_LED=2;
...
bool ppgOK=false,mpuOK=false,tempOK=false; uint16_t statusBase=0;
...
void readSensors(){
  ir=ppgOK?ppg.getIR():0; red=ppgOK?ppg.getRed():0;
...
uint16_t status(){
  ...
  if(ppgOK && ir<5000)s|=1<<ST_PPG_ABSENT;
...
void setup(){pinMode(STATUS_LED,OUTPUT);Serial.begin(BAUD_RATE);Wire.begin(SDA_PIN,SCL_PIN);analogReadResolution(12);analogSetPinAttenuation(GSR_PIN,ADC_11db);delay(300);setupSensors();calibrateIMU();setupBLE();}
void loop(){uint32_t n=millis();if(n-lp>=PPG_PERIOD_MS){lp=n;if(ppgOK){ir=ppg.getIR();red=ppg.getRed();}}if(n-li>=IMU_PERIOD_MS){li=n;if(mpuOK){sensors_event_t a,g,t;mpu.getEvent(&a,&g,&t);ax=a.acceleration.x/9.80665f-axb;ay=a.acceleration.y/9.80665f-ayb;az=a.acceleration.z/9.80665f-azb;gx=g.gyro.x*57.29578f-gxb;gy=g.gyro.y*57.29578f-gyb;gz=g.gyro.z*57.29578f-gzb;}}if(n-lg>=GSR_PERIOD_MS){lg=n;gsr=analogRead(GSR_PIN);}if(n-lt>=TEMP_PERIOD_MS){lt=n;if(tempOK){temp.requestTemperatures();temp0=temp.getTempCByIndex(0);}}if(n-lpack>=PACKET_PERIOD_MS){lpack=n;publish();}static String b;while(Serial.available()){char c=(char)Serial.read();if(c=='\n'||c=='\r'){if(b.length())command(b);b="";}else if(b.length()<48)b+=c;}}
