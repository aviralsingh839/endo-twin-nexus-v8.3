...
LOG_FILE="$PROJECT_ROOT/logs/doctor_pc.log"
echo "$(date): Launching DOCTOR_PC" | tee -a "$LOG_FILE"
if [[ ! -f "$VENV_PYTHON" ]]; then
    echo "CHRONO-PCOS environment not found at $VENV_PYTHON" | tee -a "$LOG_FILE"
    exit 1
fi
cd "$PROJECT_ROOT"
exec "$VENV_PYTHON" "$PROJECT_ROOT/launcher/doctor_pc.py" 2>&1 | tee -a "$LOG_FILE"
