...
LOG_FILE="$PROJECT_ROOT/logs/database.log"
echo "$(date): Launching DATABASE" | tee -a "$LOG_FILE"
if [[ ! -f "$VENV_PYTHON" ]]; then
    echo "CHRONO-PCOS environment not found at $VENV_PYTHON" | tee -a "$LOG_FILE"
    exit 1
fi
cd "$PROJECT_ROOT"
exec "$VENV_PYTHON" "$PROJECT_ROOT/launcher/database.py" 2>&1 | tee -a "$LOG_FILE"
