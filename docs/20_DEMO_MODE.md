...
## Overview
Strong demonstration workflow entire workflow without physical sensors DEMO/SIMULATED DATA labeled.

## Demo Patient
- Anonymous_id P12345 demo
- Age 22 BMI 23.5 USER-ENTERED
- Cycle length 28 days regular USER-ENTERED
- Symptoms: irregular cycle mild notes free text USER-ENTERED

## Simulated/Recorded Sensor
- Synthetic cohort 10 subjects 30 days 6 scenarios 60 days wrist_ppg
- Public PCOS_data.csv 541 rows
- Demo mode in patient app: Connect Sensor USB/BLE vs Demo Mode Simulated button
- Label SIMULATED or DEMO clearly
- Never label synthetic as clinical
- Never mix REAL/SYNTHETIC silently

## Signal Processing
Filtering, baseline removal, artifact detection, quality control on simulated data, quality scores 0-1, artifact detection still works.

## AI
- PCOSModule v8.3.0 pcos_associated_risk low confidence 0.75 data quality 0.85 clinical validation NOT ESTABLISHED
- SleepModule circadian_disruption_pattern moderate confidence 0.68
- Fusion multimodal risk signal
- Model transparency name/version/input/data quality/confidence/features/limitations never hide uncertainty

## Chrono-Metabolic
- Circadian rhythm experimental research quality 0.9
- Autonomic regulation derived feature quality 0.9
- Metabolic signal experimental research quality 0.8
- Fingerprint with provenance

## Ultrasound
Demo image if available, quality check UNKNOWN by design unless computed, inference requires trained model if insufficient state insufficient never fabricate percentages.

## Risk-Screening
Research / risk-screening output — not a medical diagnosis, understandable language Data quality Good, encourage professional consultation.

## Doctor Dashboard
Patient overview recent assessments quality pending reviews longitudinal, patient management create/search/open/archive/history, physiological data raw/filtered PPG HR HRV motion temp quality artifacts visualization time-series, advanced analysis circadian autonomic metabolic fingerprint multimodal AI, ultrasound all V8.3 capabilities, longitudinal comparison, doctor notes, reports professional with disclaimer.

## Report
Professional report with disclaimer Research / risk-screening output — not a medical diagnosis, model transparency, provenance, limitations.

## Every Simulated Labeled DEMO/SIMULATED
- Session label DEMO or SIMULATED
- Data label DEMO/SIMULATED
- Provider demo badge Demo Provider verification_status demo is_demo 1
- Supply demo description Demo product educational

## Science-Fair Demonstration Flow (17 Steps)
...
4. Patient Android App: Open patient app dashboard data collection status sensor status recent measurements quality cycle previous sessions notifications
...
All components share common terminology/data model/scientific foundation/identity/UI/safety language.
