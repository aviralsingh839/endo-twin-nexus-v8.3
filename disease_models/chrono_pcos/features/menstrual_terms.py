...
VOCABULARY: tuple[MenstrualTerm, ...] = (
    ...
    MenstrualTerm(
        "sleep_change",
        "Sleep-cycle change",
        ("sleep cycle", "sleep cycle change", "sleep changes", "sleep disturbance"),
        "sleep",
        "during_period",
        "handwritten research note",
    ),
...
def _candidates_for(text: str) -> List[str]:
    ...
    # Support coordinated phrases such as "pain in lower back and lower abdomen".
    if len(words) >= 2:
        prefix = " ".join(words[:2])
        if prefix in {"pain in", "feeling of"}:
            candidates.extend(f"{prefix} {part}" for part in parts[1:])
    return candidates
...
def normalize_terms(terms: Iterable[Any]) -> Dict[str, Any]:
    ...
    for item in terms or []:
        ...
        candidates = _candidates_for(text)
        matched = False

        for term in VOCABULARY:
            aliases = (
                _norm(term.key),
                _norm(term.display_name),
                *map(_norm, term.aliases),
            )
            if any(
                alias
                and any(
...
def summarize_context(
    similar_terms: Iterable[Any], dissimilar_terms: Iterable[Any]
) -> Dict[str, Any]:
