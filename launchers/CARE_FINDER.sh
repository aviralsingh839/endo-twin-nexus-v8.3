...
LOG_FILE="$PROJECT_ROOT/logs/care_finder.log"
echo "$(date): Launching CARE_FINDER" | tee -a "$LOG_FILE"
if [[ ! -f "$VENV_PYTHON" ]]; then
    echo "CHRONO-PCOS environment not found at $VENV_PYTHON" | tee -a "$LOG_FILE"
    exit 1
fi
cd "$PROJECT_ROOT"
exec "$VENV_PYTHON" "$PROJECT_ROOT/launcher/care_finder.py" 2>&1 | tee -a "$LOG_FILE"
